...
@MockitoSettings(strictness = Strictness.LENIENT) // 이거 안하면 에러남
...
public class LoginServiceTest {
    ...
    void init() {
        ...
        //회원가입 -> 가입한 회원 정보로 email, password 테스트 데이터 생성
        dbSession = MemberServiceProvider.getInstance().getSqlSession();
    ...
    void successTest() {
        ...
        //로그인처리 완료시 HttpsSession - setAttribute매서드가 호출됨
        then(session).should().setAttribute(any(), any()); // setAttribute 가 잘 호출 되었는지 확인
    }
    @Test
    @DisplayName("로그인 필수 입력 항목(이메일, 비밀번호) 체크 - 검증 실패시 BadRequestException 발생")
